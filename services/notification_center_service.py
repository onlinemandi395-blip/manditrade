...
class NotificationCenterService:
    def __init__(self, safe_drive_write_service, json_service, id_allocator_service, domain_paths_service, public_buyers_root=None) -> None:
    ...
    def public_notifications_path(self, public_buyer_id: str):
        if not self.public_buyers_root:
            raise ValueError("Public buyer notifications are not configured.")
        return self.public_buyers_root / public_buyer_id / "notifications.json"

    def list_public_notifications(self, public_buyer_id: str) -> list[dict[str, Any]]:
        path = self.public_notifications_path(public_buyer_id)
        return self.json_service.read_json(path, {"schema_version": "1.0", "notifications": []}).get("notifications", [])
    ...
    def create_public_notification(
        ...
        *,
        ...
        path = self.public_notifications_path(public_buyer_id)
        if not path.exists():
            self.safe_drive_write_service.replace_document(path, {"schema_version": "1.0", "notifications": []})
        notification = {
            "notification_id": self.id_allocator_service.allocate("notification"),
            "user_id": user_id,
            "type": notification_type,
    ...
    def update_public_status(self, public_buyer_id: str, notification_id: str, *, mark_read: bool | None = None, resolved: bool | None = None, remind_later_at: str | None = None) -> dict[str, Any]:
        path = self.public_notifications_path(public_buyer_id)
        updated: dict[str, Any] | None = None
