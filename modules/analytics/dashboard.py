...
def render_analytics_dashboard(app_context: dict) -> None:
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("Active Manufacturers", len([m for m in manufacturers if m.get("status") == "ACTIVE"]))
    col2.metric("Blocked Or Inactive", len([m for m in manufacturers if m.get("status") in {"BLOCKED", "INACTIVE"}]))
