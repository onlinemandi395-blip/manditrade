...
def render_commission_summary_dashboard(app_context: dict) -> None:
    ...
    for manufacturer in governance_service.list_manufacturers():
        ...
        private_orders = order_query_service.list_orders(manufacturer_code)
        public_orders = public_order_service.list_orders_for_seller(manufacturer_code) if public_order_service else []
        private_trade_value = 0.0
        private_admin_commission = 0.0
        private_platform_fee = 0.0
        private_manufacturer_share = 0.0
        ...
        private_row = {
            ...
            "gross_trade_value": round(private_trade_value, 2),
            "admin_commission": round(private_admin_commission, 2),
            "platform_fee": round(private_platform_fee, 2),
            "manufacturer_share": round(private_manufacturer_share, 2),
        }
        public_row = {
            "manufacturer_code": manufacturer_code,
            "manufacturer_name": manufacturer.get("business_name", manufacturer.get("manufacturer_name", "")),
        ...
        rows.extend([private_row, public_row])
        total_trade_value += private_row["gross_trade_value"] + public_row["gross_trade_value"]
